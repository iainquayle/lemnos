...
from lemnos.adapter.torch import TorchEvaluator, generate_source, Adam, SGD, OneCycleLR
...
def main():
	if (ir := model_1().compile_ir([LockedShape(3, 32, 32)], BreedIndices(), 70)) is not None: #purely for demonstration purposes
		...
		evaluator = TorchEvaluator(train_loader, validation_loader, 20, nn.CrossEntropyLoss(), accuracy_func, Adam(0.0003), OneCycleLR(.001, 20), True)
...
def model_1() -> Schema:
	...
	head_1 = SchemaNode(ShapeBound(48, None, None), None, None, Conv(3, 1), SiLU(), BatchNorm())
	head_2 = SchemaNode(ShapeBound(128, None, None), None, None, Conv(3, 1), SiLU(), BatchNorm())
	...
	tail_1 = SchemaNode(ShapeBound(512, 1), None, None, Conv(2, 0), SiLU(), BatchNorm(), debug_name="tail_1")
...
def model_2() -> Schema:
	...
	tail_1 = SchemaNode(ShapeBound(128, 1), None, None, Conv(2, 0), SiLU(), BatchNorm())
	tail_2 = SchemaNode(ShapeBound(10, 1), None, None, Full(), Softmax(), None)
