...
class LockedShape(Shape):
	...
	def squash(self, dimensionality: int) -> LockedShape:
		if dimensionality >= self.dimensionality():
			...
			return LockedShape(
				self.reverse_lower_product(dimensionality - 1), 
				*(self._shape[-(dimensionality - 1):]))
		else:
...
class ShapeBound:
	...
	def __init__(self, *bounds: tuple[int | None, int | None] | int | None) -> None:
		...
		self._bounds: list[tuple[int | None, int | None]] = [
			bound if isinstance(bound, tuple) else (bound, bound) 
			for bound in bounds] 
	...
	def scale(self, scalars: list[float]) -> ShapeBound:
		return ShapeBound(*[(int(scalar * lower) if lower is not None 
			else None, int(scalar * upper) if upper is not None else None) 
			for (lower, upper), scalar in zip(self._bounds, scalars)])
