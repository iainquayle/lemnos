...
class TestConv(unittest.TestCase):
	...
	def test_mould_output_shape_valid_upper(self) -> None:
		...
		if (shape := transform.get_output_shape(shape, OpenShape(2, 3), ShapeBound((2, 2), (1, 8)), 1, 1)) is not None:
			self.assertEqual(shape, LockedShape(2, 3))
		transform = Conv(kernel=2, stride=2, padding=0, dilation=1)
		shape = LockedShape(2, 6)
		if (shape := transform.get_output_shape(shape, OpenShape(3), ShapeBound((2, 2), (1, 8)), 1, 1)) is not None:
			self.assertEqual(shape, LockedShape(2, 3))
		shape = LockedShape(2, 6)
		if (shape := transform.get_output_shape(shape, LockedShape(2, 3), ShapeBound((1, 1000), (1, 8)), 1, 1)) is not None:
			self.assertEqual(shape, LockedShape(2, 3))
	def test_mould_output_shape_valid_lower(self) -> None:
		transform = Conv(kernel=2, stride=2, padding=0, dilation=1, groups=2)
		shape = LockedShape(2, 6)
		if (shape := transform.get_output_shape(shape, OpenShape(3), ShapeBound((2, 2), (1, 8)), 1, .5)) is not None:
			self.assertEqual(shape, LockedShape(2, 3))
		shape = LockedShape(4, 6)
		if (shape := transform.get_output_shape(shape, OpenShape(3), ShapeBound((1, 4), (1, 8)), 1, .5)) is not None:
			self.assertEqual(shape, LockedShape(2, 3))
		shape = LockedShape(2, 6)
		if (shape := transform.get_output_shape(shape, OpenShape(3), ShapeBound((1, 4), (1, 8)), 1, 2)) is not None:
			self.assertEqual(shape, LockedShape(4, 3))
		shape = LockedShape(4, 6)
		if (shape := transform.get_output_shape(shape, OpenShape(3), ShapeBound((1, 8), (1, 8)), 3, 2)) is not None:
			self.assertEqual(shape, LockedShape(6, 3))
	def test_output_shape_invalid(self) -> None:
		transform = Conv(kernel=2, stride=2, padding=0, dilation=1, groups=2)
		shape = LockedShape(2, 6)
		self.assertIsNone(transform.get_output_shape(shape, LockedShape(3, 3), ShapeBound((2, 2), (1, 8)), 1, 1))
		shape = LockedShape(3, 6)
		self.assertIsNone(transform.get_output_shape(shape, OpenShape(3), ShapeBound((2, 2), (1, 8)), 1, 1))
