...
class TestConvParameters(unittest.TestCase):
	...
	def test_mould_output_shape_valid(self) -> None:
		...
		if (shape := parameters.get_output_shape(shape, OpenShape(2, 3), Bound((2, 2), (1, 8)), Index())) is not None:
			self.assertEqual(shape, LockedShape(2, 3))
		parameters = ConvParameters(Range(1, 2), kernel=2, stride=2, padding=0, dilation=1)
		parameters.validate_dimensionality(2)
		shape = LockedShape(2, 6)
		if (shape := parameters.get_output_shape(shape, OpenShape(3), Bound((2, 2), (1, 8)), Index())) is not None:
			self.assertEqual(shape, LockedShape(2, 3))
		shape = LockedShape(2, 6)
		if (shape := parameters.get_output_shape(shape, LockedShape(2, 3), Bound((1, 1000), (1, 8)), Index())) is not None:
			self.assertEqual(shape, LockedShape(2, 3))
